...
std::tuple<bool, glm::vec3> Sphere::ray_hit_position(glm::vec3 ray_origin, glm::vec3 ray_direction)
{
    ...
    t_ca = glm::dot(position - ray_origin, ray_direction);
    
    if (t_ca < 0) {
        return {false, glm::vec3()};
    }
    
    d_sq = std::pow(glm::length(position - ray_origin), 2) - std::pow(t_ca, 2);
    ...
    return {true, ray_direction * d_v};
}

glm::vec3 Sphere::surface_normal(glm::vec3 surface_point) {
    return glm::normalize(surface_point - position);
}
